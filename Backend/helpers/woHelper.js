...
function drawFooter(doc, ctx) {
    ...
    absText(doc, `${ctx.companyName}  ·  ${ctx.docLabel}  ·  Subject to Kolkata jurisdiction`, MX, fy + 4, { size: 7, color: C.muted, width: CW * 0.6 });
    absText(doc, `Generated ${new Date().toLocaleString("en-IN")}`, MX + CW * 0.6, fy + 4, { size: 7, color: C.muted, width: CW * 0.2, align: "center" });
    absText(doc, `Page ${ctx.page} / ${ctx.totalPages}`, MX + CW * 0.8, fy + 4, { size: 7, color: C.muted, width: CW * 0.2, align: "right" });
}
...
const WI_COLS = [
    { k: "no", lbl: "Sl", fr: 0.05, a: "center" },
    { k: "desc", lbl: "Description", fr: 0.45, a: "left" },
    { k: "unit", lbl: "Unit", fr: 0.10, a: "center" },
    { k: "qty", lbl: "Quantity", fr: 0.13, a: "right" },
    { k: "rate", lbl: "Unit Rate", fr: 0.14, a: "right" },
    { k: "amt", lbl: "Amount", fr: 0.13, a: "right" },
];
WI_COLS.forEach((c) => { c.w = Math.round(c.fr * CW * 10) / 10; });
const wiDrift = CW - WI_COLS.reduce((s, c) => s + c.w, 0);
WI_COLS[WI_COLS.length - 1].w = Math.round((WI_COLS[WI_COLS.length - 1].w + wiDrift) * 10) / 10;
...
function wiTableTotals(doc, y, items, totalContractValue) {
    ...
    absText(doc, "TOTAL CONTRACT VALUE", MX + TPX, y + (RH - TFS) / 2, {
        font: F.bold, size: TFS, color: C.dark, width: CW * 0.7, lineBreak: false,
    });
    absText(doc, fmtCurrency(totalContractValue), MX + CW - 130, y + (RH - TFS) / 2, {
        font: F.bold, size: TFS + 1, color: C.accent, width: 120, align: "right", lineBreak: false,
    });
    return y + RH;
}
...
function wiTableAmountBreakdown(doc, y, {
    ...
    const valueWidth = 95;
    const rows = [
        { label: "Sub Total (Contract Value)", value: fmtCurrency(totalContractValue), bold: false },
        discount > 0 ? { label: `Discount (${discount}%)`, value: `- ${fmtCurrency(discountAmount)}`, bold: false } : null,
        gst > 0 ? { label: `GST (${gst}%)`, value: `+ ${fmtCurrency(gstAmount)}`, bold: false } : null,
        { label: "Final Payable Amount", value: fmtCurrency(finalAmount), bold: true },
    ].filter(Boolean);
    rows.forEach((row, i) => {
        ...
        absText(doc, row.label, summaryX + 8, y + (RH - (row.bold ? 10 : 8)) / 2, {
            font: row.bold ? F.bold : F.regular, size: row.bold ? 10 : 8,
...
function drawMilestonesTable(doc, y, milestones, ctx) {
    ...
    const estimatedH = TH + milestones.length * 22 + 30;
    y = space(doc, y, estimatedH, ctx);
    y = sectionLabel(doc, y, "Payment Milestones");

    const MS_COLS = [
        { lbl: "#", w: 32, a: "center" },
        { lbl: "Milestone", w: CW - 32 - 90 - 90 - 110, a: "left" },
        { lbl: "Trigger %", w: 90, a: "center" },
        { lbl: "Payment %", w: 90, a: "center" },
        { lbl: "Amount", w: 110, a: "right" },
    ];
    ...
    milestones.forEach((ms, i) => {
        const titleH = textH(doc, ms.title || "—", MS_COLS[1].w - TPX * 2, F.regular, TFS);
        const rh = Math.max(titleH + TPY * 2, 22);
        ...
        MS_COLS.forEach((col, ci) => {
            absText(doc, vals[ci], cx + TPX, y + TPY, {
                size: TFS, color: C.dark,
                width: col.w - TPX * 2, align: col.a, lineBreak: ci === 1,
            });
            cx += col.w;
        });
    ...
    strokeRect(doc, MX, tableTop, CW, y - tableTop, C.border, 0.7);
    return y + 12;
}

function countPages(wo, company, vendor, project, createdByUser) {
    let estimatedHeight = 0;

    estimatedHeight += 40 + 16;
    ...
    estimatedHeight += 30 + TH;

    const avgRowH = 22;
    estimatedHeight += wo.workItems.length * avgRowH;
    ...
    estimatedHeight += rows.length * 22;

    estimatedHeight += 30;

    if (wo.hasMilestones && wo.milestones?.length > 0) {
        estimatedHeight += 30 + TH + wo.milestones.length * 22 + 12;
    }

    estimatedHeight += 28 + 72 + 14;

    const usableH = PH - MT - MB - FZONE;
    return Math.max(1, Math.ceil(estimatedHeight / usableH));
}
...
export async function generateWOPdf(res, { wo, company, vendor, project, createdByUser }) {
    ...
    const estimatedTotal = countPages(wo, company, vendor, project, createdByUser);
    const ctx = {
        page: 1,
        totalPages: estimatedTotal,
        companyName: company?.companyName || "",
        docLabel: `WO ${wo.woNumber}`,
    };
    ...
    const itemsL3 = [
        ...
        { type: "bigheader", text: vendor?.name || "Contractor Name" },
    ...
    if (wo.description?.trim()) {
        const descW = CW;
        const descH = textH(doc, wo.description, descW, F.regular, 8) + 16;
        y = space(doc, y, descH + 24, ctx);
        y = sectionLabel(doc, y, "Scope of Work");
        absText(doc, wo.description, MX, y, { size: 8, color: C.black, width: descW });
        y += descH + 6;
    }
    ...
    if (wo.paymentTerms?.trim()) {
        doc.font(F.bold).fontSize(7.5).fillColor(C.black);
        doc.text("Payment Terms", leftInfoX, leftY, { width: leftInfoW });
        leftY += doc.heightOfString("Payment Terms", { width: leftInfoW }) + 3;
        doc.font(F.regular).fontSize(8).fillColor(C.black);
        doc.text(wo.paymentTerms.trim(), leftInfoX, leftY, { width: leftInfoW });
        leftY += doc.heightOfString(wo.paymentTerms.trim(), { width: leftInfoW }) + 8;
    }
    ...
    if (wo.specialInstructions?.trim()) {
        const siW = CW;
        const siH = textH(doc, wo.specialInstructions, siW, F.regular, 8) + 16;
        y = space(doc, y, siH + 28, ctx);
        y = sectionLabel(doc, y, "Special Instructions");
        absText(doc, wo.specialInstructions.trim(), MX, y, { size: 8, color: C.black, width: siW });
        y += siH + 6;
    }
    ...
    const totalPagesActual = ctx.page;
    doc.on("end", () => { });
...
export async function generateWCCPdf(res, { wo, company, vendor, project, completedByUser }) {
    ...
    const ctx = {
        ...
        totalPages: 1,
        companyName: company?.companyName || "",
        docLabel: `WCC for WO ${wo.woNumber}`,
    };
    ...
    const doc = new PDFDocument({
        ...
        margins: { top: MT, bottom: 4, left: MX, right: 44 },
        info: {
            Title: `WCC - WO ${wo.woNumber}`,
            Author: company?.companyName || "System",
            Subject: "Work Completion Certificate",
            Creator: "Project Management System",
        },
        compress: true,
        autoFirstPage: false,
    ...
    if (wo.completionRemarks?.trim()) {
        const rmkW = CW - 12;
        const rmkH = textH(doc, wo.completionRemarks, rmkW, F.regular, 8) + 16;
    ...
    if (wo.hasMilestones && wo.milestones?.length > 0) {
        ...
        cx = MX;
        MS_COLS2.slice(0, -1).forEach((col) => {
            cx += col.w;
            vLine(doc, cx, y, y + TH, C.border, 0.7);
        });
        y += TH;
        ...
        wo.milestones.forEach((ms, i) => {
            ...
            const rh = Math.max(maxH + TPY * 2, 22);
        ...
        strokeRect(doc, MX, tableTop2, CW, y - tableTop2, C.border, 0.7);
        y += 14;
    }
    ...
    const declW = CW - 12;
    const declH = textH(doc, declaration, declW, F.regular, 8) + 16;
    strokeRect(doc, MX, y, CW, declH, C.border, 0.5);
