...
function NACell() {
  return (
    <span className="text-xs italic text-zinc-400 dark:text-zinc-600 opacity-80">
      Not Available
    </span>
  )
}
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4 space-y-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-3 border-t border-zinc-100 dark:border-zinc-800">
        ...
        <CardField label="Source Type">
          {row.sourcetype && row.sourcetype !== NA ? row.sourcetype : <NACell />}
        </CardField>
        <CardField label="Total Amount">
          {row.totalamount && row.totalamount !== NA ? row.totalamount : <NACell />}
        </CardField>
        <CardField label="Paid Amount">
          {row.paidamount && row.paidamount !== NA ? row.paidamount : <NACell />}
        </CardField>
        <CardField label="Due Amount">
          {row.dueamount && row.dueamount !== NA ? row.dueamount : <NACell />}
        </CardField>
