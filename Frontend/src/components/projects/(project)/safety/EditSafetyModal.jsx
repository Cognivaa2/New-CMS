...
function mapInspectionToForm(inspection) {
    ...
    const keycloakId =
        typeof window !== "undefined" ? localStorage.getItem("keycloakId") || "" : ""
    ...
    return {
        entries: (inspection.entries || []).map((entry) => ({
            ...
            inspectedBy: entry.inspectedBy?.keycloakId || entry.inspectedByRaw || keycloakId,
...
export default function EditSafetyModal({
    ...
    useEffect(() => {
        if (inspection) setForm(mapInspectionToForm(inspection))
    }, [inspection])
    ...
    const validate = useCallback(() => {
        ...
        for (let i = 0; i < form.entries.length; i++) {
            ...
            if (!e.inspectedBy?.trim()) {
                toast.error("Inspector is required — please refresh")
                return false
            }
        }
        return true
    ...
    const loading = isEditLoading || !form
