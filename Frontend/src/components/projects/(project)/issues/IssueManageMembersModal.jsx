...
import { fetchAssignedUsers } from "@/app/(companyname)/projects/[projectId]/issues/api"
...
export default function IssueManageMembersModal({
    isOpen,
    onClose,
    issue,
    projectUsers = [],
    ...
    useEffect(() => {
        ...
        fetchAssignedUsers(issue.id, { page: 1, limit: 100 }, controller.signal)
            .then(({ users }) => {
    ...
    const availableUsers = filteredResults.filter(
        (u) => !assignedIds.has(getUserKey(u))
    )
    async function handleAdd(user) {
        ...
        try {
            if (onAssign) await onAssign(user)
            setMembers((p) => [
                ...p,
                {
    ...
    async function handleConfirmUnassign() {
        ...
        try {
            if (onUnassign) await onUnassign(confirmTarget)
    ...
    return (
        <>
            ...
            >
                ...
                />
                <div
                    className={`relative w-full max-w-135 bg-white dark:bg-[#121212] rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                        visible
                ...
                >
                    ...
                    <div className="px-6 pb-5 mt-2">
                        <h2 className="text-[18px] font-sfpro-bold text-gray-900 dark:text-white tracking-tight">
                            Manage Issue Members
                        </h2>
                        <p className="text-sm text-gray-500 dark:text-[#888] mt-1 font-sfpro">
                            {issue?.title || issue?.issue
                                ? `Assign or remove team members for "${issue.title || issue.issue}"`
                                : "Assign or remove team members for this issue"}
                        </p>
                    </div>
                                ...
                                members.map((member, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <Tooltip
                                                content="Remove from this issue"
                                                side="top"
                                            >
                                                <button
            ...
            <DeleteModal
                ...
                description={`${confirmTarget?.name || "This member"} will be removed from this issue.`}
                itemName=""
