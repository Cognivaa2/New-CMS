...
export default function SubTaskManageMembersModal({
    isOpen,
    onClose,
    subtask,
    projectUsers = [],
    ...
    // Initialize members from subtask
    useEffect(() => {
        if (isOpen && subtask) {
            const subtaskMembers = (subtask.assignedTo || []).map(user => ({
                keycloakId: user.keycloakId || user.userId || user.id || user._id,
                id: user.id || user._id,
    ...
    // Filter project users based on search query
    ...
    const assignedIds = new Set(members.map(m => getUserKey(m)))
    const availableUsers = filteredResults.filter(u => !assignedIds.has(getUserKey(u)))
    ...
    async function handleConfirmUnassign() {
        ...
        const uid = getUserKey(confirmTarget)
        ...
        try {
            ...
            setMembers(p => p.filter(m => getUserKey(m) !== uid))
            membersChangedRef.current = true
            setConfirmTarget(null)
        } catch (err) {
            console.error(err.message)
    ...
    return (
        <>
                ...
                <div className={`relative w-full max-w-135 bg-white dark:bg-[#121212] rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${visible ? "translate-y-0 opacity-100 scale-100" : "translate-y-6 opacity-0 scale-95"}`}>
                                            ...
                                            {availableUsers.map((user, index) => {
                                                ...
                                                return (
                                                    <li key={uid || index} className="flex items-center gap-3 px-4 py-2.5 transition-colors">
                                                        ...
                                                        <Tooltip content="Assign to this subtask" side="top">
                                                            <button
                    ...
                    <div className="px-6 pb-5 mt-2">
                        <h2 className="text-[18px] font-sfpro-bold text-gray-900 dark:text-white tracking-tight">
                            Manage Subtask Members
                        </h2>
                        <p className="text-sm text-gray-500 dark:text-[#888] mt-1 font-sfpro">
                            {subtask?.title ? `Assign or remove team members for "${subtask.title}"` : "Assign or remove team members for this subtask"}
                        </p>
                    </div>
                                ...
                                members.map((member, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <Tooltip content="Remove from this subtask" side="top">
                                                <button
            ...
            <DeleteModal
                ...
                description={`${confirmTarget?.name || "This member"} will be removed from this subtask.`}
                itemName=""
