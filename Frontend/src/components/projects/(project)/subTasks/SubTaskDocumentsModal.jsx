...
import { X, Search, Loader2, FileText, Folder } from "lucide-react"
import { fetchSubTaskDocuments, debounce } from "@/app/(companyname)/projects/[projectId]/phases/[phaseId]/[taskId]/api"
...
function DocumentRow({ doc, onView }) {
  ...
  return (
    <button
      onClick={() => onView(doc)}
      className="cursor-pointer w-full flex items-center gap-3 px-2 py-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors duration-150 text-left group"
    >
...
export default function SubTaskDocumentsModal({ isOpen, onClose, subtask, taskId, projectId }) {
  ...
  useEffect(() => {
    if (!isOpen || !taskId || !subTaskId) return
  ...
  }, [isOpen, taskId, subTaskId, currentPage, debouncedQuery])
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === "Escape" && isOpen && !isViewerOpenRef.current) onClose()
    }
  ...
  const handleViewDoc = useCallback(
    (doc) => {
      setViewerDoc({ ...doc, taskId, subTaskId, projectId })
      setIsViewerOpen(true)
    },
    [taskId, subTaskId, projectId]
  )
  ...
  const handleBackdropClick = useCallback(() => {
    if (!isViewerOpenRef.current) onClose()
  }, [onClose])
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        >
          ...
          <div className="px-6 pb-5 mt-2">
            <h2 className="text-lg font-sfpro-bold text-gray-900 dark:text-white tracking-tight">
              SubTask Documents
            </h2>
            <p className="text-sm text-gray-500 dark:text-[#8d8d8d] mt-1 font-sfpro line-clamp-1">
              {subtask?.title || "View and manage documents linked to this subtask"}
            </p>
          </div>
          ...
          <div className="px-6 py-5">
            ...
            <div
              ...
              className="min-h-60 max-h-80 overflow-y-auto -mx-2 px-2 space-y-0.5 scrollbar-hide"
            >
              {isLoading ? (
                  ...
                  <div className="text-center">
                    <p className="text-[13.5px] text-gray-500 dark:text-[#888] font-sfpro">
                      {debouncedQuery
                        ...
                        : "No documents linked to this subtask"}
                    </p>
                    {debouncedQuery && (
                      <button
                        onClick={clearSearch}
                        className="mt-2 text-[12.5px] font-sfpro-medium text-gray-400 dark:text-[#666] hover:text-gray-700 dark:hover:text-white underline underline-offset-2 transition-colors"
                      >
                        Clear search
                      </button>
                    )}
                ...
                <>
                  ...
                  {isFetchingMore && (
                    <div className="flex justify-center py-3">
                      <Loader2
                        size={16}
                        className="text-gray-400 dark:text-[#555] animate-spin"
                      />
                    </div>
                  )}
                </>
