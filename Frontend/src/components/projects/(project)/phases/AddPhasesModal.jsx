...
function DatePickerField({ label, placeholder, colSpan, value, onChange, required, minDate, maxDate, highlightStart, highlightEnd }) {
  return (
    <div className={colSpan}>
...
export default function AddPhaseDrawer({ open, onClose, onSave, projectStartDate, projectEndDate }) {
  ...
  const handleSubmit = async () => {
    ...
    if (projectStartDate && start < new Date(projectStartDate.setHours(0, 0, 0, 0))) {
      toast.error("Invalid Start Date", { description: `Phase cannot start before project start (${formatDate(projectStartDate)})` })
      return
    }
    if (projectEndDate && end > new Date(projectEndDate.setHours(23, 59, 59, 999))) {
      toast.error("Invalid End Date", { description: `Phase cannot end after project end (${formatDate(projectEndDate)})` })
      return
    }
    setSubmitting(true)
    try {
          ...
          <div className="flex items-center justify-center px-8 pt-5 pb-5 shrink-0 transition-colors">
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm font-sfpro-medium text-gray-500 dark:text-[#71717a] mt-1 transition-colors">
                Define a new phase for your project timeline
              </p>
            </div>
          </div>
          <div
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-5">
              ...
              <DatePickerField
                ...
                highlightStart={projectStartDate}
                highlightEnd={projectEndDate}
              />
              <DatePickerField
                label="End Date"
                colSpan="col-span-2 md:col-span-1"
