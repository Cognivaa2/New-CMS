...
const STATUS_CONFIG = {
    ...
    PartiallyDelivered: { label: "Part. Delivered", icon: Truck, className: "text-amber-600 bg-amber-50 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20" },
                ...
                <table className="w-full min-w-135 border-collapse text-[12px]">
                    <thead>
                        <tr className="bg-gray-50 dark:bg-[#18181b] border-b border-gray-100 dark:border-[#27272a]">
                            {["#", "Material", "Unit", "Qty", "Unit Price", "Disc%", "GST%", "Total"].map((h, i) => (
                                <th
                                    key={i}
                    ...
                    <tbody className="bg-white dark:bg-[#0d0d0d]">
                        {items.map((item, idx) => (
                            ...
                            >
                                ...
                                <td className="px-3 py-3 text-gray-500 dark:text-[#71717a]">{item.unit || "—"}</td>
                                ...
                                <td className="px-3 py-3 text-right text-gray-500 dark:text-[#71717a]">
                    ...
                    <tfoot>
                        <tr className="border-t-2 border-gray-100 dark:border-[#27272a] bg-gray-50 dark:bg-[#18181b]">
                            <td colSpan={7} className="px-3 py-3 text-right text-[11px] font-sfpro-bold text-gray-500 dark:text-[#71717a] uppercase tracking-wide">
                                Grand Total
                            </td>
...
export default function ViewPOModal({ open, onClose, po, isLoading = false }) {
                    ...
                    <div className="flex items-start justify-center px-8 lg:px-10 pt-5 pb-4 shrink-0">
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-sm font-sfpro-medium text-gray-500 dark:text-[#71717a] mt-1">
                                View all details of this purchase order
                            </p>
                        </div>
                    </div>
                                ...
                                <div className="flex flex-col gap-6 px-8 lg:px-10 pb-8">
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <p className="text-[13px] text-[#71717a] dark:text-[#a1a1aa] font-sfpro">
                                            {po.vendorName} · {po.createdAt}
                                        </p>
                                    </div>
                                    <div className="h-px bg-gray-100 dark:bg-[#252525]" />
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
                                        <InfoRow icon={FileText} label="PO Number" value={po.poNumber} />
                                        <InfoRow icon={Package} label="Vendor" value={po.vendorName} />
                                        <InfoRow
                                            icon={Calendar}
                                        ...
                                        <InfoRow
                                            ...
                                            value={po.specialInstructions}
                                        />
                                        <InfoRow
                                            icon={Package}
                                            label="Items"
                                            value={String(po.items?.length ?? 0)}
                                        />
                                        <InfoRow
                                            icon={Package}
                                            label="Total Order Value"
                                            value={`₹${(po.totalOrderValue || 0).toLocaleString("en-IN")}`}
                                        />
                                        <InfoRow
                                            icon={Calendar}
                                            label="Created At"
                                    ...
                                    {po.status === "Rejected" && po.rejectionRemarks && (
                                        <>
                                            <div className="h-px bg-gray-100 dark:bg-[#252525]" />
                                    ...
                                    {po.status === "Cancelled" && po.cancellationRemarks && (
                                        <>
                                            <div className="h-px bg-gray-100 dark:bg-[#252525]" />
                                    ...
                                    <div className="flex flex-col gap-3">
                                        <SectionHeader
                                            title={`Order Items (${po.items?.length ?? 0})`}
                                        />
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                            ...
                                            <UserBlock
                                                ...
                                                user={po.updatedBy}
                                                date={ po.updatedAt ? formatDate(po.updatedAt) : null}
                                            />
                                        </div>
                                    </div>
                                </div>
