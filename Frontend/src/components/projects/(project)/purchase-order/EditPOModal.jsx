...
export default function EditPOModal({
  ...
  useEffect(() => {
    ...
    setForm({
      mrId: po.mrId || "",
      mrObject: po.mrId ? { _id: po.mrId, mrNumber: po.poId } : null,
      vendorId: po.vendorId || "",
      vendorObject: po.vendorId
        ? { _id: po.vendorId, name: po.vendorName }
        : null,
  ...
  const handleSubmit = async () => {
    ...
    if (hasEmptyQty) {
      toast.error("All items need a valid quantity")
      return
    }

    const hasNegativePrice = form.items.some((i) => i.unitPrice < 0)
    if (hasNegativePrice) {
        ...
        <div className="w-full bg-white dark:bg-[#09090b] border-t-2 border-gray-200 dark:border-[#27272a] rounded-t-2xl max-h-[92dvh] lg:max-h-[70dvh] flex flex-col">
          ...
          <div className="flex flex-col items-center px-8 pt-5 pb-4 shrink-0">
            ...
            <p className="text-sm font-sfpro-medium text-gray-500 dark:text-[#71717a] mt-1">
              {po?.status === "Rejected"
                ? "This PO was rejected — update and resubmit"
                : `Editing ${po?.poNumber || "PO"}`}
            </p>
          </div>
          {isEditLoading || !form ? (
            <div className="flex-1 px-8 lg:px-20 xl:px-36 pt-7 pb-10 space-y-4">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-10 rounded-lg bg-gray-100 dark:bg-[#1c1c1c] animate-pulse"
                />
              ))}
            </div>
          ) : (
