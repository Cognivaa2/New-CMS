...
import { POFormContent } from "./POFormFields"

const INITIAL_FORM = {
  mrId: "", mrObject: null,
  vendorId: "", vendorObject: null,
...
export default function AddPOModal({ open, onClose, onSave, projectId }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setVisible(false)
      const t = setTimeout(() => { setMounted(false); setForm(INITIAL_FORM) }, 420)
      return () => clearTimeout(t)
    }
  }, [open])

  useEffect(() => {
    const fn = (e) => { if (e.key === "Escape" && open && !submitting) onClose() }
    window.addEventListener("keydown", fn)
    return () => window.removeEventListener("keydown", fn)
  }, [open, submitting, onClose])

  const handleSubmit = async () => {
    if (!form.mrId) { toast.error("Select an MR"); return }
    if (!form.vendorId) { toast.error("Select a vendor"); return }
    if (!form.items?.length) { toast.error("Add at least one item"); return }
    const hasEmptyQty = form.items.some((i) => !i.orderedQuantity || i.orderedQuantity <= 0)
    if (hasEmptyQty) { toast.error("All items need a valid quantity"); return }
    const hasInvalidMaterial = form.items.some((i) => !i.materialMasterId)
    if (hasInvalidMaterial) {
      toast.error("Select a material for each item")
      return
    }

    const hasEmptyPrice = form.items.some((i) => i.unitPrice === undefined || i.unitPrice < 0)
    if (hasEmptyPrice) {
        ...
        <div className="w-full bg-white dark:bg-[#09090b] border-t-2 border-gray-200 dark:border-[#27272a] rounded-t-2xl max-h-[92dvh] lg:max-h-[70dvh] flex flex-col">
          ...
          <div className="flex flex-col items-center px-8 pt-5 pb-4 shrink-0">
            ...
            <p className="text-sm font-sfpro-medium text-gray-500 dark:text-[#71717a] mt-1">
              Raise a PO from an approved material requisition
            </p>
          </div>
