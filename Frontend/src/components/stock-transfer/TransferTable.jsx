...
function ProjectCell({ project }) {
  if (!project || typeof project !== "object" || project.name === NA)
    return <NACell />
  return (
    <div className="min-w-0">
      <p className="text-[13px] font-sfpro-medium text-zinc-800 dark:text-zinc-100 truncate max-w-40">
        {project.name}
      </p>
      {project.code && (
...
function renderCell(row, header) {
  ...
  switch (key) {
    case "fromproject":
    case "toproject":
      return <ProjectCell project={value} />
    ...
    case "materials":
      return (
        <TextCell
          value={value}
          className="max-w-60 line-clamp-2 leading-snug"
        />
      )
...
function TableRow({ row, headers }) {
  return (
    <tr className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
      {headers.map((h, j) => (
        <td key={j} className="px-6 py-1 align-middle">
          {renderCell(row, h)}
        </td>
      ))}
    </tr>
...
function MobileCard({ row }) {
  const fromName =
    row.fromproject && typeof row.fromproject === "object"
      ? row.fromproject.name
      : NA
  const toName =
    row.toproject && typeof row.toproject === "object"
      ? row.toproject.name
      : NA
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4 space-y-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-[13px] font-sfpro-medium text-zinc-800 dark:text-zinc-100 truncate max-w-32">
              {fromName !== NA ? fromName : <NACell />}
            </p>
            <ArrowRight className="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500 shrink-0" />
            <p className="text-[13px] font-sfpro-medium text-zinc-800 dark:text-zinc-100 truncate max-w-32">
              {toName !== NA ? toName : <NACell />}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-3 border-t border-zinc-100 dark:border-zinc-800">
        <CardField label="Materials">
          {row.materials && row.materials !== NA ? (
            <span className="line-clamp-2 leading-snug">{row.materials}</span>
          ) : (
            <NACell />
          )}
        </CardField>
        <CardField label="Total Items">
          {row.totalitems && row.totalitems !== NA ? (
            row.totalitems
          ) : (
            <NACell />
          )}
        ...
        {row.reason && row.reason !== NA && (
          <div className="col-span-2">
            <CardField label="Reason">
              <span className="line-clamp-2 leading-snug">{row.reason}</span>
            </CardField>
          </div>
        )}
      </div>
      ...
      {row.approvedby && typeof row.approvedby === "object" && (
        <div className="pt-3 border-t border-zinc-100 dark:border-zinc-800">
          <span className="text-[10px] uppercase tracking-wider font-sfpro-bold text-zinc-400 dark:text-zinc-500">
            Approved By
          </span>
          <div className="mt-1.5">
            <UserCell user={row.approvedby} />
          </div>
        </div>
      )}
      ...
      {row.rejectedby && typeof row.rejectedby === "object" && (
        <div className="pt-3 border-t border-zinc-100 dark:border-zinc-800">
          ...
          {row.rejectionremarks && row.rejectionremarks !== NA && (
