...
const GRN_STATUS_CONFIG = {
  ...
  "GRN Pending":
    "text-amber-600 bg-amber-50 border border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20",
  "Not Applicable":
    "text-zinc-500 bg-zinc-100 border border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700",
}
...
function GRNStatusBadge({ value }) {
  if (!value || value === NA) return <NACell />
  const cls =
    GRN_STATUS_CONFIG[value] ||
    "text-zinc-500 bg-zinc-100 border border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700"
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-sfpro-bold whitespace-nowrap ${cls}`}
    >
      {value}
    </span>
  )
...
function renderCell(row, header) {
  ...
  switch (key) {
    ...
    case "grnstatus":
      return <GRNStatusBadge value={value} />
    case "orderedby":
    case "approvedby":
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4 space-y-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[14px] font-sfpro-bold text-zinc-900 dark:text-zinc-100 truncate">
            {row.poid || "Not Available"}
          </p>
          <p className="text-[12px] text-zinc-400 dark:text-zinc-500 truncate mt-0.5">
            {row.project && row.project !== NA ? row.project : "Not Available"}
          </p>
        </div>
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          <StatusBadge status={row.status} />
          <GRNStatusBadge value={row.grnstatus} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-3 border-t border-zinc-100 dark:border-zinc-800">
        ...
        <CardField label="Total Amount">
          {row.totalamount && row.totalamount !== NA ? row.totalamount : <NACell />}
        </CardField>
        ...
        <CardField label="Delivery By">
          {row.deliveryby && row.deliveryby !== NA ? row.deliveryby : <NACell />}
        </CardField>
