...
const INITIAL = {
  ...
  address: "",
  description: "",
  legalDetails: { gstin: "", pan: "", registrationNo: "" },
...
export default function AddVendorModal({ open, onClose, onSave }) {
  ...
  async function handleSubmit() {
    if (!form.name?.trim()) {
      toast.error("Missing Required Fields", { description: "Vendor name is required." })
      return
    }
    if (!form.phone?.trim()) {
      toast.error("Missing Required Fields", { description: "Phone number is required." })
      return
    }
    if (!form.email?.trim()) {
      ...
      >
        <div className="w-full bg-white dark:bg-[#09090b] border-t-2 border-gray-200 dark:border-[#27272a] rounded-t-2xl max-h-[88dvh] lg:max-h-[80dvh] flex flex-col transition-colors duration-300">
          ...
          <div className="flex items-center justify-center px-8 pt-5 pb-5 shrink-0">
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm font-sfpro-medium text-gray-500 dark:text-[#71717a] mt-1 transition-colors">
                Fill in the details to add a vendor to your company
              </p>
            </div>
          </div>
          <VendorFormContent
          ...
          <div className="shrink-0 flex items-center justify-end gap-2 px-8 py-4">
            ...
            >
              {submitting ? "Creating…" : "Add Vendor"}
            </button>
          </div>
