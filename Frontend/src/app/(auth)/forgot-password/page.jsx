  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(data.title || "OTP Sent", {
        description: data.description || "If this email is registered, an OTP has been sent",
      })
      router.push(`/verify-otp-forgot-password?email=${encodeURIComponent(email.trim().toLowerCase())}`)
    } catch (err) {
          ...
          <div className="w-full flex flex-col items-center gap-3 mb-5">
            <div className="w-full flex items-center justify-center">
              <input
                ...
                disabled={loading}
                className="w-full xl:w-97 bg-black/22 rounded-[10px] border border-[#292929] px-6 py-4 text-sm font-sfpro text-white placeholder:text-[#CFCFCF] outline-none focus:border-white/20 transition-all duration-200 appearance-none disabled:opacity-50 disabled:cursor-not-allowed"
              />
            </div>
