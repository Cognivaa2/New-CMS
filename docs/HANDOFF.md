...
5. **File uploads require R2 (was causing 500s):** all uploads go through `utils/uploadToR2.utils.js` → an S3 client pointed at Cloudflare R2. If the `R2_*` env vars are unset, every multipart POST/PATCH 500s with `"Resolved credential object is not valid"` (requests without a file are unaffected). Configured against R2 bucket **`newcms`** (public URL `https://pub-b684e7f0fce74db8a8d7190543413cbb.r2.dev`); the S3 keys live only in the server `Backend/.env`. See `deploy/env/backend.env.example`.
